...
namespace M3C.Finance.BinanceSdk
{
    public partial class BinanceClient
    {
        ...
        public async Task<bool> KeepAliveUserDataStream(string listenKey) {
            var response = await SendRequest<JObject>("userDataStream", ApiVersion.Version1, ApiMethodType.ApiKey, HttpMethod.Put,
        ...
        public async Task<bool> CloseUserDataStream(string listenKey) {
            var response = await SendRequest<JObject>("userDataStream", ApiVersion.Version1, ApiMethodType.ApiKey, HttpMethod.Delete,
                new Dictionary<string, string> {{"listenKey", listenKey}});
