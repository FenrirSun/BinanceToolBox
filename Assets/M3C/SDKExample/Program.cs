...
namespace M3C.Finance.Binance.Example
{
    class Program
    {
        ...
        static void PublicEndpointExamples() {
            ...
            Console.WriteLine(
                $"AggTradeId: {aggregateTrades[0].AggregateTradeId} Price: {aggregateTrades[0].Price}  Quantity: {aggregateTrades[0].Quantity}  WasMaker: {aggregateTrades[0].WasBuyerTheMaker}");
            ...
            foreach (var item in allBookTickers) {
                Console.WriteLine(
                    $"{item.Symbol} : AskPrice/Quantity {item.AskPrice} / {item.AskQuantity} BidPrice/Quantity {item.BidPrice} / {item.BidQuantity}");
            }
        }
        ...
        static void SignedEndpointExamples() {
            ...
            var orderResult = restClient
                .NewOrder("NEOBTC", OrderSide.Sell, OrderType.Limit, TimeInForce.GoodUntilCanceled, 100m, 0.05m, true, "order1").Result;
            ...
            Console.WriteLine(
                $"Status: {orderQueryResult.Status} Executed Qty: {orderQueryResult.ExecutedQuantity} Orig Qty: {orderQueryResult.OriginalQuantity}");
